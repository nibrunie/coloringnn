...
class DiffusionModel(keras.Model):
    ...
    def reverse_diffusion_keep(self, initial_noise, diffusion_steps):
        # reverse diffusion = sampling
        num_images = initial_noise.shape[0]
        step_size = 1.0 / diffusion_steps
        ...
        for step in range(diffusion_steps):
            ...
            image_diffusions.append(pred_images)
            # this new noisy image will be used in the next step

        return image_diffusions
    ...
    def plot_images(self, epoch=None, logs=None, num_rows=3, num_cols=6, saveFile=None):
        ...
        initial_noise = tf.random.normal(shape=(num_images, self.image_size, self.image_size, 3))
        # list of img_stack, each stack contains the same diffusion step of a different stream
        generated_images = self.reverse_diffusion_keep(initial_noise, plot_diffusion_steps)
        generated_images = [self.denormalize(img_stack) for img_stack in generated_images]
        #generated_images = self.generate(
        #    num_images=num_rows * num_cols,
        #    diffusion_steps=plot_diffusion_steps,
        #)
        if saveFile:
            for step, stack in enumerate(generated_images):
                for i, img in enumerate(stack):
                    predicted_image = img.reshape((self.image_size, self.image_size) + (3,))
                    cv2.imwrite(f"{saveFile}-{i}-{step}.png", (predicted_image.numpy() * 256).astype("uint8"))
            ...
            for row in range(num_rows):
                for col in range(num_cols):
                    ...
                    plt.imshow(generated_images[-1][index])
                    plt.axis("off")
            plt.tight_layout()
            plt.show()
            plt.close()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model-type", type=str, choices=["basic"],
                   default="basic",
    ...
    parser.add_argument("--plot-output", type=str, default="./results/",
    ...
    checkpoint_path = join(args.model_path) # , "checkpoints")
    ...
    if not args.train is None and not args.dataset is None:
        ...
        for i in range(args.plot_images):
            model.plot_images(saveFile=join(args.plot_output, f"hallucinated-landscape-{i}"))
