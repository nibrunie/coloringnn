...
def generate_medium_model(GRAY_DIM=(128, 128)):
    ...
    x = layers.Conv2D(256, 3, activation='relu', padding='same', name='conv2d_5')(x)
    x = layers.Conv2D(256, 3, activation='relu', padding='same', name='conv2d_6')(x)
    conv2d_7 = layers.Conv2D(256, 3, activation='relu', name='conv2d_7')(x)
    x = layers.MaxPool2D((2,2),name="maxpool_3")(conv2d_7)
    x = layers.Conv2D(512, 3, activation='relu', padding='same', name='conv2d_8')(x)
    x = layers.Conv2D(512, 3, activation='relu', padding='same', name='conv2d_9')(x)
    x = layers.Conv2D(512, 3, activation='relu', padding='same', name='conv2d_10')(x)

    x = layers.Conv2D(256, 1, activation="relu", padding='same', name="conv2d_1x1")(x)
    x = layers.UpSampling2D(name="up_sampling_1")(x)
    x = layers.Add()([x, conv2d_7])
    x = layers.Conv2D(128, 3, activation="relu", padding='same', name="conv2d_recons_2")(x)
